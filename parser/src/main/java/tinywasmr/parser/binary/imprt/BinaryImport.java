...
public record BinaryImport(String module, String name, BinaryImportDesc description) {
	...
	public static BinaryImport parse(InputStream stream) throws IOException {
		...
		case TYPE_TABLE -> new BinaryTableImport(StreamReader.parseTableType(stream));
		case TYPE_MEM -> new BinaryMemoryImport(StreamReader.parseMemoryType(stream));
		case TYPE_GLOBAL -> new BinaryGlobalImport(StreamReader.parseGlobalType(stream));
